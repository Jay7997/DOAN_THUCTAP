...
class ProductService
{
    ...
    protected function callRawPhpApi(string $endpoint)
    {
        try {
            ...
            Log::info("Calling API", ['endpoint' => $endpoint, 'fullUrl' => $fullUrl]);
            ...
            Log::error("API call failed", ['endpoint' => $endpoint, 'status' => $response->status(), 'response' => $response->body()]);
        } catch (\Exception $e) {
    ...
    public function fetchData($limit = 100, $type = 'all', $category = null)
    {
        ...
        Log::info("Fetching data", ['type' => $type, 'limit' => $limit, 'category' => $category, 'cacheKey' => $cacheKey]);
        ...
        if ($type === 'all' || $type === 'news') {
            Log::info("Fetching news data", ['newsId' => $this->newsId, 'limit' => $limit]);
            $result = $this->callRawPhpApi("module.Tintuc.asp?id={$this->newsId}&sl={$limit}");

            if (!is_array($result) || !isset($result[0]['data']) || !is_array($result[0]['data'])) {
                Log::error("Failed to fetch news from API", [
                    'endpoint' => "module.Tintuc.asp?id={$this->newsId}&sl={$limit}",
                    'result' => $result
                ]);
            } else {
                Log::info("Successfully fetched news from API", [
                    'count' => count($result[0]['data']),
                    'newsId' => $this->newsId
                ]);
                foreach ($result[0]['data'] as &$item) {
                    $item['hinhdaidien'] = $this->fixImageUrl($item['hinhdaidien'] ?? null);
        ...
        Log::info("Cached new data", ['cacheKey' => $cacheKey, 'productsCount' => count($response['products']), 'newsCount' => count($response['news'])]);
        return $response;
    }
    ...
    public function fetchNewsDetail($newsId)
    {
        ...
        if (isset($detail[0])) {
            ...
            $item['hinhdaidien'] = $this->fixImageUrl($item['hinhdaidien'] ?? null);
            Cache::put($cacheKey, $item, 86400);
            Log::info("Successfully fetched news detail", ['newsId' => $newsId]);
            return $item;
        }

        Log::error("Failed to fetch news detail", [
            'newsId' => $newsId,
            'detail' => $detail
        ]);
